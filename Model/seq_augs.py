...
class Seq_Augmentor(nn.Module):
    def __init__(self, aug_ratio, preset=-1):
        super().__init__()
        self.aug_ratio = aug_ratio
        self.aug = preset
        self.Reorder = Reorder(beta = aug_ratio)
        self.Mask = Mask(gamma=aug_ratio)
        self.Crop = Crop(tao=aug_ratio)

    def forward(self, sequence):
        self.aug = 3#np.random.randint(3)
        if self.aug == 0:
            sequence = self.Reorder(sequence)
        elif self.aug == 1:
